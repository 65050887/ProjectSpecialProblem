...
function DriveImg({ url, alt, className, style, ...props }) {
  ...
  useEffect(() => setSrc(thumb || direct || FALLBACK_IMG), [thumb, direct]);
...
export default function HomeUser() {
  ...
  // ใช้ base ไม่มี /api เหมือน Home.jsx
  const API_BASE =
  ...
  function goSearchUser(params = {}) {
    ...
    navigate(qs ? `/user/search?${qs}` : "/user/search");
  }

  function goDormDetail(d) {
    const id = d?.id || d?.dorm_id || d?.dorm_code;
    if (!id) return;
    // รายละเอียดหอใช้หน้าเดิมได้
    navigate(`/user/dorm/${id}`);
  }
  ...
  const stats = useMemo(() => {
    ...
    return [
      { num: totalDormitoryNum ? totalDormitoryNum.toLocaleString("th-TH") : "-", label: "Total Dormitory" },
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 items-center gap-2">
            <div>
              ...
              <p className="max-w-[539px] mb-6 text-[14px] leading-[25px] text-justify" style={{ color: PRIMARY }}>
                Search through dormitories around KMITL with detailed information, photos, and availability.
              ...
              <div className="mt-4 max-w-[540px]">
                <div className="flex items-center gap-3 rounded-[12px] px-4 py-3" style={{ border: `1px solid ${PRIMARY}` }}>
                  ...
                  <input
                    ...
                    onKeyDown={(e) => e.key === "Enter" && goSearchUser(q ? { q } : {})}
        ...
        <div className="mx-auto w-full max-w-[1190px] px-6 py-[50px]">
          <div className="flex flex-col items-center gap-[42px]">
            ...
            <div className="w-full grid grid-cols-1 md:grid-cols-4 gap-8">
              {categories.map((c) => (
                <div key={c.title} className="w-full h-[400px] rounded-[30px] bg-white px-5 py-6 flex flex-col items-center">
              ...
              {!reviews.length && (
                <div className="col-span-1 md:col-span-3 rounded-[30px] bg-white px-[28px] py-6">
                  ...
                  <p className="text-black text-[14px] mt-2">
                    (ฝั่ง User) เดี๋ยวเราไปเปิดให้กดรีวิวใน SearchUser / DormDetail ต่อได้
                  </p>
                </div>
              )}
            </div>
